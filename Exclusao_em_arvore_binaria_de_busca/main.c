#include <stdio.h>
#include <stdlib.h>
#include <string.h>

typedef struct sNoA {
    int chave;
    struct sNoA *esq;
    struct sNoA *dir;
} TNoA;

TNoA *busca(TNoA *nodo, int chave) {
    if (nodo == NULL) {
        return NULL;
    }
    else if (chave == nodo->chave) {
        return nodo;
    }
    else if (chave > nodo->chave) {
        return busca(nodo->dir, chave);
    }
    else {
        return busca(nodo->esq, chave);
    }
}



TNoA *maior_a_esquerda(TNoA *raiz){
    TNoA *aux;
    aux = raiz;
    while (aux->dir != NULL) {
        aux = aux->dir;
    }
    return aux;
}


TNoA *exclui(TNoA *raiz, int chave) {
/*Consultar as funções "busca" e "busca_anterior" para saber como o nó a ser excluido e seu nó anterior
são encontrados */
    if (raiz == NULL) {
        return raiz;
    }
    if (chave > raiz->chave) {
        raiz->dir = exclui(raiz->dir, chave);
    }
    else if (chave < raiz->chave) {
        raiz->esq = exclui(raiz->esq, chave);
    }
    else {
        TNoA *aux;

        if (raiz->dir == NULL) {
            aux = raiz->esq;
            free(raiz);
            return aux;

        }
        else if (raiz->esq == NULL) {
            aux = raiz->dir;
            free(raiz);
            return aux;
        }

        aux = maior_a_esquerda(raiz->esq);
        raiz->chave = aux->chave;
        raiz->esq = exclui(raiz->esq, aux->chave);
    }
    return raiz;


}

void imprime(TNoA *nodo, int tab) {
    for (int i = 0; i < tab; i++) {
        printf("-");
    }
    if (nodo != NULL) {
        printf("%d\n", nodo->chave);
        imprime(nodo->esq, tab + 2);
        printf("\n");
        imprime(nodo->dir, tab + 2);
    } else printf("vazio");
}

TNoA *insere(TNoA *no, int chave) {
    if (no == NULL) {
        no = (TNoA *) malloc(sizeof(TNoA));
        no->chave = chave;
        no->esq = NULL;
        no->dir = NULL;
    } else if (chave < (no->chave))
        no->esq = insere(no->esq, chave);
    else if (chave > (no->chave)) {
        no->dir = insere(no->dir, chave);
    } else {
        printf("Inserção inválida! ");
        exit(1);
    }
    return no;
}

int main(void) {

    /* A função main lê os dados de entrada, cria a árvore e chama a função solicitada no problema
     * depois imprime o resultado solicitado
     * ELA NÃO DEVE SER MODIFICADA
     * */
    TNoA *raiz;
    raiz = NULL;



    char l[100];
    char delimitador[] = "-";
    char *ptr;
    int valor;

    /* lê valores para criar a arvore
     * valores devem ser informados separados por traço
     * exemplo: 1-3-5-2-7-9-21-6 */
    scanf("%s", l);
    //quebra a string de entrada
    ptr = strtok(l, delimitador);
    while(ptr != NULL) {
        valor = atoi(ptr);
        raiz = insere(raiz, valor);
        ptr = strtok(NULL, delimitador);
    }

    //le valor a ser excluido
    scanf("%d", &valor);
    //Chama função
    raiz = exclui(raiz, valor);
    imprime(raiz, 0);
};