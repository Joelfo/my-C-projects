...
TLista* exclui(TLista* li, int valor) {
    TLista* p_inicial;
    TLista* p_atual;
    TLista* anterior_ao_atual;
    TLista* prox_ao_atual;
    p_inicial = li;
    p_atual = p_inicial;
    anterior_ao_atual = NULL;
    for(p_atual = li; p_atual != NULL; p_atual = p_atual->prox){
        prox_ao_atual = p_atual->prox;
        if(p_atual->info == valor){
            if(anterior_ao_atual == NULL || prox_ao_atual == NULL){
                free(li);
                free(p_inicial);
                return NULL;
            }
            else if(anterior_ao_atual = NULL){
