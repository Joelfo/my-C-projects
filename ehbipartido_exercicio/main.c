...
int ehbipartido(TGrafo *g) {
    ...
    TGrafo *grafo_vizinho;
    TVizinho *percorre_vertice;
    TGrafo *percorre_grafo;
    percorre_grafo = g;
    while(percorre_grafo != NULL) {
        percorre_vertice = percorre_grafo->prim_vizinho;
        while (percorre_vertice != NULL) {
            grafo_vizinho = busca_vertice(g, percorre_vertice->id_vizinho);
            if (grafo_vizinho->cor == g->cor) {
                return 0;
            } else {
                percorre_vertice = percorre_vertice->prox;
            }
        }
