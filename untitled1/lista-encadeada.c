...
void altera(TLista* li, int vantigo, int vnovo) {
    //O auxiliar é usado para percorrer a lista encadeada "li"
    TLista* aux;
    for(aux = li ; aux != NULL ; aux = aux->prox) {
        //Caso o valor seja encontrado em algum ponto da lista encadeada "li", ele será substituído
        if (aux->info == vantigo) {
            aux->info = vnovo;
            return;
        }
        /*Caso isso não ocorra, o for irá simplesmente percorrer toda a lista e a função terminará sem executar
        mais nenhum comando*/
    }
}
